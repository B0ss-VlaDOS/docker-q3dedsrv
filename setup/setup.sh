...
then
  ...
  then
    ...
    then
      ...
      # try with su
      # this doesn't mean X11 is not available. Maybe xsu is broken / not available.
      printf "$SU_MESSAGE\n"
      try_run -absolute /bin/su root -c "export DISPLAY=$DISPLAY;sh `pwd`/setup.sh -auth"
      status="$?"
      # su returns 1 if auth failed - otherwise it returns process exit code
      # check the exit code to decide if we should go on as user
      if [ "$status" -eq 0 ]
      then
        # the auth command was properly executed
        exit 0
      fi
      # the su auth failed or was cancelled
      if ( [ "$status" -eq 1 ] && [ "$GET_ROOT" -eq 2 ] )
      then
        # we don't want to even start the setup if not root
        echo "Please run this installation as the super user"
        exit 1
      fi
    elif ( [ "$status" -eq 2 ] && [ "$GET_ROOT" -eq 2 ] )
    then
      # the auth failed or was explicitely canceled
      # we don't want to even start the setup if not root
      echo "Please run this installation as the super user"
      exit 1
    fi
...
if [ $status -ne 0 ] && [ $status -ne 3 ]; then  # setup.gtk couldn't connect to X11 server - ignore
  ...
  # if this executed through an auth loop with su (not xsu), we need correct exit code returned
  exit 0
fi
exit 0
